...
import (
	...
	"github.com/danielpickens/tesseract/utils"
...
func (connector *Connector) CloneScript() {
	cloneScriptUrl := fmt.Sprintf("%s/scripts/clone", connector.url)

	ok := false
	var err error
	for !ok {
		var resp *http.Response
		if resp, err = utils.Post(cloneScriptUrl, "application/json", nil, connector.client, config.Config.License); err != nil || resp.StatusCode != http.StatusOK {
			if _, ok := err.(*url.Error); ok {
				break
			}
			log.Warn().Err(err).Msg("Failed sending the POST clone script to Hub. Retrying...")
		} else {
			log.Debug().Msg("Reported POST clone script to Hub.")
			return
		}
		time.Sleep(DefaultSleep)
	}
...
func (connector * Connector) ModifyActiveScript() { 
	modify:= fmt.Sprintf("%s/scripts/modify", connector.url)
	x =: false
	var err error

	for !x {
		var resp *http.Response
		if resp, err = utils.Post(modify, "application/json", nil, connector.client, config.Config.License); err != nil || resp.StatusCode != http.StatusOK {
			if _, ok := err.(*url.Error); ok {
				break
			}
			log.Warn().Err(err).Msg("Failed sending the POST modify script to Hub. Retrying...")
		} else {
			log.Debug().Msg("Reported POST modify script to Hub.")
			return
		}
		time.Sleep(DefaultSleep)
	}
