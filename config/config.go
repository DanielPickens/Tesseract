...
import (
	...
	"github.com/danielpickens/tesserect/misc"
	"github.com/danielpickens/tesserect/misc/version"
	"github.com/danielpickens/tesserect/utils"
...
// InitConfig calls the initFlag function for each flag in the command and loads the config file.
	...
	if err := loadConfigFile(&Config, utils.Contains([]string{
		...
		if !os.IsNotExist(err) {
			return fmt.Errorf("invalid config, %w\n"+
				"you can regenerate the file by removing it (%v) and using `tesseract config -r`", err, ConfigFilePath)
		}
	}
...
// Creates the default config struct for application
